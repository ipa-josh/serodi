...
def main(do_setup):
    ...
    with sm:
		...
		with sq:
			...
			if do_setup:
				smach.Sequence.add('Mapping1', states.initialization.ROSLaunch('serodi_mapping',mapping_launch))
				...
				smach.Sequence.add('MappingKill', states.initialization.ROSKill('serodi_mapping',mapping_launch))
			...
			smach.Sequence.add('WaitL', states.interaction.Wait(45))
		...
		if do_setup:
			...
			smach.StateMachine.add('Localization2_Global2', states.movement.AutoLocalize(sss),
				transitions={'success': 'Main2', 'failed': 'failure'})
