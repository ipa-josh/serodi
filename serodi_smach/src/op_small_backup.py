...
def main():
    ...
    sm.userdata.data = {'op_small_poses':{}, 'op_small_scans':{}}
    ...
    with sm:
        ...
        smach.StateMachine.add('LoadYaml', states.interaction.LoadYaml('op_small_poses','../config/op_small_backup_poses.yaml', sm), 
                               transitions={'success':'LoadYaml2', 'failed':'failure'})
        smach.StateMachine.add('LoadYaml2', states.interaction.LoadYaml('op_small_scans', '../config/op_small_backup_scans.yaml', sm), 
                               transitions={'success':'SelectLight', 'failed':'failure'})

        #select random light
        transitions={'none':'success'}
        for light in sm.userdata.data['op_small_poses']['lights']:
        ...
        for light in sm.userdata.data['op_small_poses']['lights']:
			...
			with sq:
				...
				smach.Sequence.add('MoveTo_'+l, states.movement.MoveOnPathRel(sss,[light['pose'][0]],[sm.userdata.data['op_small_scans'][l][0]]))
			...
			with sq:
				...
				smach.Sequence.add('MoveToHome_'+l, states.movement.MoveOnPathRel(sss,[light['pose'][1]],[sm.userdata.data['op_small_scans'][l][1]]))
